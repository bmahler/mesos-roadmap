...
def main():
    ...
    for epic in epics:
        nodes.add(epic['key'])
        print(json.dumps(epic, indent=4))
        fields = epic['fields']
        if fields.has_key('issuelinks'):
            if fields['issuelinks']:
                for issuelink in fields['issuelinks']:
                    if issuelink.has_key('inwardIssue') and issuelink['type']['name'] == 'Blocker':
                        edges.add((epic['key'], issuelink['inwardIssue']['key']))
                    if issuelink.has_key('outwardIssue') and issuelink['type']['name'] == 'dependent':
                        edges.add((epic['key'], issuelink['outwardIssue']['key']))
    ...
    for edge in edges:
        start, end = edge
        f.write("  " + start.replace("-", "_") + ' -> ' + end.replace("-", "_") + ';\n')        
    f.write('}\n')
